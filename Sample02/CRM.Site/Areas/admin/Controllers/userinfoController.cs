...
namespace CRM.Site.Areas.admin.Controllers
{
    ...
    public class userinfoController : BaseController
    {
        ...
        {
            string[] useridwkid = id.Split('-');
            ViewBag.uid = useridwkid[0];
            ViewBag.wkid = useridwkid[1];
        ...
        public ActionResult getRolelist(string id) 
        {
            ...
            int iid=id.AsInt();
            //查询
            var list = roleSer.QueryWhere(c => c.eDepID == wkid).Select(c => new { c.rID, c.rName ,ischecked=roleList.Any(d=>d.rID==c.rID)});
